    ...
    regParser(key, (data: any[]): any => {
        ...
        try {
            ...
            for (let i = 1; i < data.length; i++) {
                ...
                let local = hasLocal && {} as { [key: string]: any };
                for (let j = 0; j < headersRaw.length; j++) {
                    let head = headersRaw[j];
                ...
                forEach(ins, i - 1, key as string, dict, idkey as string);
...
function getParserOption(idkey: string | 0 = "id", type?: CfgDataType) {
    let dict: any, forEach: { (t: any, idx: number, key: string, dict: any, idkey: string): any };
    if (idkey == "" || idkey == 0) {
    ...
    regParser(key, (bytes: ByteArray) => {
        ...
        try {
            ...
            while (bytes.readAvailable && count--) {
                ...
                forEach(obj, i++, key as string, dict, idkey as string);
