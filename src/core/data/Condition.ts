...
function getValue(node: ConditionNode, context: ConditionCheckContext): any {
    const { op, value } = node;
    switch (op) {
...
let conditionValueSolver: ConditionValueSolver = Pipe;
...
export type ConditionFuncSolver = { (nodes: ConditionNode[], context: ConditionCheckContext): any };
...
export type ShowConditionTip = { (msgs: string[]): any };

let showTip: ShowConditionTip = Noop;
