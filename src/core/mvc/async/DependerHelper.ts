...
export class DependerHelper {
	 ...
	 * @param args
	 */
	public constructor(host: any, callback: Function, ...args: any[]) {
		this._host = host;
	...
	public addDepend(async: IAsync) {
		if (async.isReady) {
			...
			pushOnce(this._unreadyDepender, async);
	...
	protected readyHandler(async: IAsync) {
		removeFrom(this._unreadyDepender, async);
		this.check();
	}
	...
	public check() {
		if (!this._uncheck) {
			this._uncheck = true;
			App.nextTick({ callback: this._check, context: this, unique: true });
		}
	}
