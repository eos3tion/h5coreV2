...
import { INetHandler } from "../../net/NetRouter";
...
export abstract class Service extends Proxy {
    ...
    protected reg() {
        ...
        for (let i = 0; i < args.length; i += 3) {
            let cmd = args[i];
            let ref = args[i + 1];
            let handler = args[i + 2] as INetHandler;
            handler = handler.bind(this);
            if (Array.isArray(cmd)) {
                for (let i = 0; i < cmd.length; i++) {
                    doReg(ns, cmd[i], handler, ref);
                }
            } else {
                doReg(ns, cmd, handler, ref);
            }
        }
        function doReg(ns: NetService, cmd: number, handler: INetHandler, ref: Key) {
     ...
     * @param cmds 注册的指令
     */
    protected regMsg(ref: string | number, ...cmds: any[]) {
        let ns = this._ns;
        for (let cmd of cmds) {
            ns.regReceiveMSGRef(cmd, ref);
        }
    ...
    protected removeHandler(cmd: number, handler: INetHandler) {
     ...
     */
    protected send(cmd: number, data?: any, msgType?: string | number, limit = 200) {
