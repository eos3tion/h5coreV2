...
export class ModuleManager {
    ...
    /**
     * Key      {number} 模块id<br/>
     * Value    {ViewDele[]} 绑定在同一个模块上的按钮的数组
     */
    _bindedIOById: { [id: string]: ViewDele[] };
     ...
     */
    _hByType: { [type: number]: ModuleHandler };
     ...
     * Value    {IModuleHandler}    模块处理器
     */
    _hById: { [id: string]: ModuleHandler };

    /**
     * Key      {ViewDele}   绑定的交互对象  
     * Value    {number}                模块id
     */
    _ioBind: Map<ViewDele, string | number>;
    ...
    init() {
        ...
        this._unopens = [];
        this._hById = {};
        this._ioBind = new Map<ViewDele, string | number>();
        on(EventConst.ModuleNeedCheckShow, this.check, this);
    }
    ...
    doCheckLimits() {
        ...
        App.callLater(200, { callback: this.checkLimits, context: this, unique: true });
    }
    ...
    *
    */
    bindButton(id: string | number, io: ViewDele, eventType: Key = ViewDeleEvent.TouchTap): void {
        ...
        if (!this.isModuleShow(id)) {
            io.visible = false;
            pushOnce(_unshowns, id);
        }
        else {
            removeFrom(_unshowns, id);
        }
    }
     ...
     *
     */
    ioHandler(event: DataEvent): void {
        this.toggle(this._ioBind.get(event.target));
    }
    ...
    check(): void {
        ...
        App.callLater(200, this._check, this);
    }
     ...
     */
    regModuleOpen(mid: Key, callback: $Callback) {
     ...
     * @param {$Callback} callback 
     */
    regModuleShow(mid: Key, callback: $Callback) {
        let cfg = this._allById[mid];
        if (cfg) {
            if (this.isModuleShow(cfg)) {
                callback.execute();
