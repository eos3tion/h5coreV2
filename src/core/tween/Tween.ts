...
export class Tween extends EventEmitter {
    ...
    private initialize(target: any, props: TweenOption, pluginData: any, manager: TweenManager) {
        this.target = target;
        ...
        if (props) {
            ...
            props.onChange && this.on(EventConst.TweenChange, props.onChange, props.onChangeObj);
            if (props.override) {
     ...
     * is `false`).
     */
    setPosition(value: number, actionsMode: TweenActionMode = TweenActionMode.LOOP) {
        if (value < 0) {
            value = 0;
        }
        // normalize position:
        ...
        this.dispatch(EventConst.TweenChange);
        return end;
    }


    private _runActions(startPos: number, endPos: number, includeStart?: boolean) {
    ...
    private _addAction(o: TweenAction) {
        o.t = this.duration;
    ...
    wait(duration: number, passive?: boolean) {
        ...
        var o = clone(this._curQueueProps)
     ...
     * @return {Tween} This tween instance (for chaining calls).
     */
    to(props: Object, duration?: number, ease?: IEaseFunction) {
        if (isNaN(duration) || duration < 0) {
            duration = 0;
        }
        return this._addStep({ d: duration || 0, p0: clone(this._curQueueProps), e: ease, p1: clone(this._appendQueueProps(props)) });
    }
     ...
     * @return {Tween} This tween instance (for chaining calls).
     */
    call(callback: Function, thisObj?: any, ...params: any[]) {
