import { Tween, TweenOption } from "./Tween";
...
export class TweenManager {
     ...
     */
    _plugins: { [index: string]: ITweenPlugin[] } = {};
    /**
    ...
    * @static
    */
    get(target: any, props?: TweenOption, pluginData?: any, override?: boolean) {
        if (override) {
     ...
     * @static
     */
    tick(delta: number, paused?: boolean) {
        if (!this._tweens.length) {
            return;
        }
        const tweens = this._tweens.concat();
        for (let i = tweens.length - 1; i >= 0; i--) {
            let tween = tweens[i];
     ...
     */
    _register(tween: Tween, value?: boolean) {
     ...
     */
    installPlugin(plugin: ITweenPlugin, properties: string[]) {
