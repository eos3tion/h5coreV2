 ...
 * @template T
 * @param clazz 对象定义
 * @param addInstanceRecycle 是否将回收方法附加在实例上，默认将回收方法放在实例
 */
export function recyclable<T>(clazz: Creator<T> & { _pool?: RecyclablePool<T> }, addInstanceRecycle?: boolean, max?: number): Recyclable<T> {
    ...
    if (!pool) {
        if (addInstanceRecycle) {
            ...
            pool = new RecyclablePool(clazz, max);
