...
export function getPBUtils() {
    ...
    function regStruct(msgType: Key, struct: PBStruct) {
        ...
        let def = defDict[msgType as number];
        if (def) {
            regDef(struct, def);
        }
        //检查处理默认值
                        ...
                        if (typeof struct != "object") {
                            ...
                            while (true) {//防止出现多重关联的情况 
                                ...
                                if (struct == null) {//或者undefiend
                                    return DEBUG && ThrowError(`添加ProtoBuf字典有误，请检查数据`);
                                }
                                if (typeof struct == "object") {
                                    break;
                                }
                                if (~cycle.indexOf(struct)) {
                                    return DEBUG && ThrowError(`添加ProtoBuf字典有误，出现循环的配置`);
                                }
                                cycle.push(struct);
                            }
                        }
     ...
     * @param msgType 
     * @param bytes
     * @param debugOutData  调试时，使用的输出数据
     * @returns {ByteArray} 
     */
    function writeTo(msg: any, msgType: Key | PBStruct, bytes?: ByteArray, debugOutData?: any): ByteArray {
        if (msg == undefined) {
            return;
        }
        let struct = typeof msgType == "object" ? msgType : structDict[msgType];
