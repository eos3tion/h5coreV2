 ...
 *
 */
export function setDirtyHandler(handler: { (substring: string): string }) {
    replaceDirty = handler;
}
/**
 * 由于脏字文件使用ajax读取，可能存在跨域问题，所以在H5中使用javascript方式加载
 */
export function loadDirtyWord(url: string, split = "\n") {
    Res.loadRes({ uri: "$dirty", url, type: ResItemType.Text }, Callback.get(item => {
        let $dirty = item.data;
        if ($dirty) {
 ...
 *
 */
export const initDirty = initFilterstring;
/**
 * 文字过滤
 * @author 3tion
 */
