...
import { IDrawInfo } from "./AnimationDefine";
...
export class UnitResource {
    ...
    public decodeData(data: any) {
        let _datas: { [index: string]: egret.Texture[][] } = {};
        let w = 0, h = 0;
         ...
         * 从数据中获取纹理
         */
        function getTextureFromImageData(data: any) {
    ...
    public loadData() {
        if (this.state == RequestState.UnRequest) {
            ...
            Res.load(this.uri, this.url, Callback.get(this.dataLoadComplete, this), this.qid);
        }
    }
    ...
    getUrl(uri: string) {
        return ConfigUtils.getResUrl(uri + App.webpExt);
    }
