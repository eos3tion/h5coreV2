...
import { EgretRectangle } from "../../EgretTemp";
import { bindDrag, DragDele, DragOption, looseDrag, stopDrag } from "../core/Drag";
import { ScrollBar } from "../creator/ScrollBarCreator";
...
export class Scroller extends egret.EventDispatcher {
    ...
    public showBar() {
        if (this._useScrollBar && !this.alwaysShowBar) {
            App.tweenManager.get(this._scrollbar, null, null, true).to({ alpha: 1 }, 500);
        }
    }

    public hideBar() {
        if (this._useScrollBar && !this.alwaysShowBar) {
            App.tweenManager.get(this._scrollbar, null, null, true).to({ alpha: 0 }, 1000);
        }
    }
    ...
    public doScrollContent(sub: number) {
        ...
        if (old != pos) {
            ...
            content.dispatch(EventConst.ScrollPosChange);
        }
