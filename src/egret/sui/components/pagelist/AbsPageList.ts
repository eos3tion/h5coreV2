...
export abstract class AbsPageList<T, R extends ListItemRender<T>> extends egret.EventDispatcher {
     ...
     */
    public forEach(handle: { (data: T, render: R, idx: number, ...args: any[]): any }, ...otherParams: any[]) {
     ...
     * @returns 
     */
    public find(handle: { (data: T, render: R, idx: number, ...args: any[]): boolean }, ...otherParams: any[]) {
        let datas = this._data;
        let renders = this._list;
        let len = this._dataLen;
        for (let i = 0; i < len; i++) {
    ...
    protected onTouchItem(e: egret.TouchEvent) {
        ...
        this.dispatch(EventConst.ItemTouchTap, render);
    }
    ...
    protected changeRender(render: R, index?: number) {
        ...
        if (!render.unelectable) {
            ...
            this.onChange();
            this.dispatch(EventConst.ItemSelected);
        }
    }
