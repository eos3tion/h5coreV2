...
import { Empty, SharedPoint } from "../../../../core/constants/Shared";
...
import { getDescriptorMap } from "../../../../core/utils/ObjectUtils";
...
const tmp = [] as egret.DisplayObject[];
const tmpRect = new egret.Rectangle;
...
export class PageList<R extends ListItemRender<T>, T = R["data"]> extends AbsPageList<R> {
    ...
    protected init(option: PageListOption) {
        option = option || Empty as PageListOption;
        ...
        Object.defineProperties(con, getDescriptorMap({
            measuredHeight: {
                get() {
                    return self._h;
                }
    ...
    layout(type: LayoutType) {
        ...
        if (suiRawRect) {
            ...
            getLayoutPos(_w, _h, width, height, type, SharedPoint);
            if (_w < width) {
                x += SharedPoint.x;
            }
            if (_h < height) {
    ...
    protected _get(index: number) {
        ...
        if (!render) {
            ...
            render.on(EventConst.ItemTouchTap, this.onTouchItem, this);
        }
        render.index = index;
        return render;
    }
    ...
    protected onSizeChange() {
        if (!this._sizeChanged) {
            this._sizeChanged = true;
            App.callLater(100, { callback: this.reCalc, context: this });
        }
    }
    ...
    private moveScroll(render: R) {
        ...
        const tm = App.tweenManager;
        tm.removeTweens(this);
        if (this.useTweenIndex) {
            ...
            let tween = tm.get(this).to(result, 500, Ease.quadOut);
            if (scroller) {
                scroller.showBar();
    ...
    protected _removeRender(item: R) {
        ...
        item.off(EventConst.ItemTouchTap, this.onTouchItem, this);
