...
import { BaseData, ComponentData, createSui, initSuiBaseData, suiTFCreator } from "../core/SuiResManager";
import { TextData } from "./TextFieldCreator";
...
export class MovieClip extends Component {
    ...
    constructor(data: any, framesData: MCFrameData[]
        ...
        if (compsData) {
            ...
            for (let i = 0; i < compsData.length; i++) {
                let dis = createSui(compsData[i], suiData, this);
                if (dis instanceof egret.DisplayObject) {
                    comps[i] = dis;
                }
            }
        } else {
            comps = EmptyArray as egret.DisplayObject[];
        }
        this.compData = comps;
    ...
    public play(frame?: number) {
        ...
        this.playing = true;
        this._nt = App.getNow() + this.timePerFrame;
    ...
    protected render(frame: number) {
        ...
        if (frameData && frameData.key != this.currentFrame) {//当前帧是否和要渲染的关键帧相同
            ...
            for (let dat of frameData.data) {
                ...
                if (idx == -1) {//-1的一定有pData
                    comp = createSui(pData as ComponentData, suiData, this);
                } else {
                    comp = dict[idx];
                    if (comp instanceof egret.DisplayObject) {
                        this.addChild(comp)
                        if (pData) {//调整基础属性
                            initSuiBaseData(comp, pData);
                        }
                        if (comp instanceof egret.TextField) {//如果是文本框，特殊处理
                            if (!textData) {
                                textData = comp.rawTextData;
                            }
                            suiTFCreator.initTextData(comp, textData);
                        }
                    }
                }
            }
