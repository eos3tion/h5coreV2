...
import { Bin, ShortSideBinPacker } from "../../core/utils/binpacker/BinPacker";
import { TextureSheetConst, TextureSheet, DynamicTexture, getTextureSheet } from "./TextureSheet";
...
export function getDynamicTexSheet(size?: number, path?: Path2D, pathColor?: string) {
            ...
            if (_cur) {
                ...
                if (tex) {
                    ...
                    if (bin) {
                        tex.$bin = undefined;
                        //将位置还原给装箱数据
                        removeFrom(packer.usedRects, bin);
                        pushOnce(packer.freeRects, bin);
                    }
                }
            }
        },
    ...
    function bind(uri: Key, tex: DynamicTexture): DynamicTexture {
        //检查是否已经加载过
        let _cur = dict[uri];
        if (_cur) {//已经有，不做处理
            return tex;
