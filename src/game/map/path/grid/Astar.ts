...
import { GridMapInfo } from "./GridMapInfo";
...
export class Astar implements PathFinder {
     ...
     * @memberOf PathFinder
     */
    public getPath(fx: number, fy: number, tx: number, ty: number, callback: Callback<PathFinderCallback>, opt?: PathFinderOption) {
        const map = this._map;
        if (!map) {
        ...
        let { sync, max } = opt || Empty as PathFinderOption;
